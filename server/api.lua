...
CreateThread(function() if (GetCurrentResourceName() ~= "Sav-Permissions") then Debug("Error Some Scripts Sav Releases Might Not Work If This Is Renammed",true) end  end)
...
function Debug(Type, Error)
    ...
    if Error then
        print(string.format("^1 Sav: %s (Error: %s)", Type, Error))
    else
        print(string.format("^1 Sav: %s", Type))
    end
end
...
function UserMetaTable:RequestUserData()
    ...
    if Cooldowns[self.source] and Cooldowns[self.source] > currentTime then
        ...
        ServerApi.Data.chatMessage(self.source, '[Sav Api]', string.format("Must Wait %s Seconds Before Requesting Api Again", math.floor(remainingTime)))
        return
    end
    ...
    if responseData then
        ...
        if responseDataTable and next(responseDataTable.roles) ~= 0 then
            ...
            TriggerClientEvent('Sav:ReturnData', self.source, self, ServerApi.Data.Debugs)
        else
...
RegisterServerEvent('Sav:SendPerms', function()
    local source = source
